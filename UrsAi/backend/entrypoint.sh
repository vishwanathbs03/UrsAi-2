...
#
# The script does NOT run database migrations. Atlas AI creates the
# schema from SQLAlchemy metadata on first connect, so the application
# handles schema bootstrap itself; adding a separate migration step here
# would couple the image to a specific migration tool that is not
