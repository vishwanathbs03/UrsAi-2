...
function buildGeneralOverview(bundle: AssistantBundle): AssistantResponse {
  ...
  return {
    body:
      ...
      `- Recommendations: ${recommendations.recommendations.length} (${recommendations.summary.critical_count} critical, ${recommendations.summary.high_count} high)\n` +
      `- Roadmap items: ${roadmap.items.length}, projected total duration ${roadmap.summary.total_estimated_duration}\n` +
      `- Rule firings: ${rules.summary.total_firings} across ${rules.summary.categories_with_firings} categories\n` +
      `- ${insightLine}\n\n` +
    ...
    sources: [
      ...
      { topic: "Recommendations", detail: `${recommendations.recommendations.length} items queued.` },
      { topic: "Roadmap", detail: `${roadmap.items.length} items sequenced.` },
      { topic: "Rules", detail: `${rules.summary.total_firings} active firings.` },
      ...(decision
        ? [
            {
