...
function buildInsights(args: BuildInsightsArgs): InsightItem[] {
  ...
  // Flatten rule firings into a single map for O(1) lookup.
  ...
  // Index recommendations by their supporting_rule_ids
  // so an insight can pull its related recommendations
  // through the rule bridge.
  const recsByRuleId = new Map<string, RecommendationItem[]>();
  for (const r of recommendations.recommendations) {
    for (const rid of r.supporting_rule_ids) {
  ...
  // Index roadmap items by recommendation_id.
  const roadmapByRecId = new Map<string, RoadmapItem>();
  for (const item of roadmap.items) {
    roadmapByRecId.set(item.recommendation_id, item);
  }

  return decision.decision.insights.map((insight) => {
    const supportingRules: RuleFiring[] = [];
    for (const rid of insight.supporting_rule_ids) {
      const f = ruleMap.get(rid);
      if (f) supportingRules.push(f);
    }
    // Sort supporting rules by priority weight (Critical first).
    supportingRules.sort(
      (a, b) => PRIORITY_ORDER[a.priority] - PRIORITY_ORDER[b.priority],
    );

    // Aggregate related recommendations through the rules.
    ...
    // Fallback: if the insight has no supporting rules but
    // its raw category matches a recommendation category,
    // surface the highest-priority recommendations in that
    // category. This is the same join pattern the action
    // board uses; no new business logic.
    if (relatedRecs.length === 0) {
      const fallback = recommendations.recommendations
    ...
    // Priority: highest-priority supporting rule, or Low fallback.
    const priority: RuleFiring["priority"] =
      supportingRules[0]?.priority ?? "Low";

    // Category: classify the raw AI category; if no match,
    // pick the most common category across the related
    // recommendations.
    ...
    return {
      ...
      priority,
      supportingRuleIds: insight.supporting_rule_ids,
      supportingArticleIds: insight.supporting_article_ids,
      supportingRules,
