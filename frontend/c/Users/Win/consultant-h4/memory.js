...
// features/assistant/memory.ts
var memory_exports = {};
__export(memory_exports, {
  topicForKind: () => topicForKind,
  useAssistantMemory: () => useAssistantMemory
});
module.exports = __toCommonJS(memory_exports);
var import_react = __toESM(require_react());
var MAX_RECENT = 12;
function useAssistantMemory() {
  const [entries, setEntries] = (0, import_react.useState)([]);
  const [lastActionPlanId, setLastActionPlanId] = (0, import_react.useState)(null);
  const remember = (0, import_react.useCallback)(
    (raw) => {
      ...
      setEntries((prev) => {
        ...
        const next = existing ? prev.map((p) => p.id === entry.id ? entry : p) : [...prev, entry];
  ...
  const forget = (0, import_react.useCallback)(() => {
    setEntries([]);
    setLastActionPlanId(null);
  }, []);
  const topicCounts = (0, import_react.useMemo)(() => {
    const acc = {};
  ...
  const topicsAnswered = (0, import_react.useMemo)(() => {
  ...
  const recallForTopic = (0, import_react.useCallback)(
    (topic) => entries.filter((e) => e.topic === topic),
    [entries]
  );
  const recallRecent = (0, import_react.useCallback)(
